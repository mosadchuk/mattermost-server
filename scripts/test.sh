...
$GO test $GOFLAGS -run=$TESTS $TESTFLAGS -v -timeout=$TIMEOUT -covermode=$COVERMODE -coverpkg=$PACKAGES_COMMA -exec $DIR/test-xprog.sh "github.com/mattermost/mattermost-server/v5/mlog" 2>&1 > >( tee output )
