...
func (s SqlChannelStore) GetCRTUnfixedChannelMembershipsAfter(channelID, userID string, count int) ([]model.ChannelMember, error) {
	...
	args := []interface{}{userID, channelID, count}
	getUnfixedCMQuery := `
			SELECT ChannelMembers.*
			FROM ChannelMembers, Channels
			WHERE ChannelId = Id AND (ChannelMembers.UserId, ChannelMembers.ChannelId) > (?, ?) AND Channels.TotalMsgCountRoot > ChannelMembers.MsgCountRoot
	...
	if err := s.GetReplicaX().Select(&cms, getUnfixedCMQuery, args...); err != nil {
